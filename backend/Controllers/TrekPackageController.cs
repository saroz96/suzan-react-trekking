...
namespace backend.Controllers
{
    ...
    public class TrekPackageController : ControllerBase
    {
        ...
        private static TrekPackageResponseDto MapToResponseDto(TrekPackage package)
        {
            ...
            // Safely handle collections with null checks
            var sliderImages = package.SliderImages?
                .Where(s => s.IsActive)
                .Select(s => new TripSliderImageDto
                {
                    Id = s.Id,
                ...
                .ToList() ?? new List<TripSliderImageDto>();

            var galleryImages = package.GalleryImages?
                .Where(g => g.IsActive)
                .Select(g => new TripGalleryImageDto
                {
                    Id = g.Id,
                ...
                .ToList() ?? new List<TripGalleryImageDto>();

            var itinerary = package.Itinerary?
                .Select(i => new TripItineraryDayDto
                {
                    Id = i.Id,
                    DayNumber = i.DayNumber,
                    Title = i.Title,
                ...
                .ToList() ?? new List<TripItineraryDayDto>();

            var costIncludes = package.CostIncludes?
                .Select(c => new TripCostIncludeDto
                {
                    Id = c.Id,
                ...
                .ToList() ?? new List<TripCostIncludeDto>();

            var costExcludes = package.CostExcludes?
                .Select(c => new TripCostExcludeDto
                {
                    Id = c.Id,
                    Description = c.Description ?? string.Empty,
                    Category = c.Category,
                ...
                .OrderBy(c => c.DisplayOrder)
                .ToList() ?? new List<TripCostExcludeDto>();

            var faqs = package.Faqs?
            ...
            var departureDates = package.DepartureDates?
                .Where(d => d.IsAvailable)
                .Select(d => new TripDepartureDateDto
                {
                    Id = d.Id,
                    StartDate = d.StartDate,
                    EndDate = d.EndDate,
                ...
                .ToList() ?? new List<TripDepartureDateDto>();

            var groupDiscounts = package.GroupDiscounts?
                .Where(g => g.IsActive)
                .Select(g => new GroupDiscountDto
                {
                    Id = g.Id,
                    MinTravelers = g.MinTravelers,
                    MaxTravelers = g.MaxTravelers,
                ...
                .OrderBy(g => g.MinTravelers)
                .ToList() ?? new List<GroupDiscountDto>();

            // Calculate review statistics
            var approvedReviews = package.Reviews?
                .Where(r => r.IsActive && r.IsApproved)
                .ToList() ?? new List<TrekPackageReview>();
                ...
                RatingDistribution = new Dictionary<int, int>
        {
            ...
            var recentReviews = approvedReviews
            ...
            return new TrekPackageResponseDto
            {
                ...
                CreatedAt = package.CreatedAt,
                UpdatedAt = package.UpdatedAt,
                IsBestSeller = departureDates.Any(d => d.IsBestSeller),
                IsTopSeller = departureDates.Any(d => d.IsTopSeller),
                HasGuaranteedDeparture = departureDates.Any(d => d.IsGuaranteed),
